...
describe('POST /api/translate', () => {
  ...
  it('翻訳成功時にのみ利用量カウントが増える', async () => {
    const env = createMockEnv();
    env.DB._tables.users.push({ id: 'user-inc', email: 'inc@b.com', plan: 'standard' });

    const token = await createTestJWT('user-inc', 'inc@b.com', env.JWT_SECRET);
    const request = new Request('https://mylabeln.com/api/translate', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    // usageテーブルは空の状態で開始
    expect(env.DB._tables.usage.length).toBe(0);

    const response = await onRequestPost(createMockContext(request, env));
    const { status } = await parseResponse(response);

    expect(status).toBe(200);
    // 成功後にusageレコードが追加されていること
    expect(env.DB._tables.usage.length).toBe(1);
    expect(env.DB._tables.usage[0].user_id).toBe('user-inc');
  });
